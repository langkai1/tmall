...

{
    ...
    let containers=document.querySelectorAll(".dd");//引入div标签container

    tips.forEach(function(ele,index){//列出tips的元素
        if(index>0){
             ele.onclick=function(){//鼠标单击效果

            let ot=containers[index-1].offsetTop;//当前对象到上级层左边的距离减去50像素
            let now=ot-document.documentElement.scrollTop;
            let speed=(ot-now)/8;
            let time=0;
            let t=setInterval(function(){
                ...
                now+=speed;
                document.documentElement.scrollTop=now-200;
            },25);
        }
        }
